/* ============================================================
   CADENCE TOYBOX — app.js
   Same trainer engine as v1/v2, candy-coated UI.
   ============================================================ */
...
const MODES = {
  ...
  filler: {
    ...
    prompts: [
      'Describe your morning routine. Every filler word feeds the ghost.',
...
const CANDY = ['#ff4d00', '#6c1ee0', '#2b46f0', '#f2055c', '#00a551', '#e6b800'];
...
function resetSessionUI(seconds) {
  ...
  $('#recordBtnLabel').textContent = '🎙️ Go';
  $('#recordBtn').classList.remove('recording');
  $('#liveTranscript').innerHTML = '<span class="lt-placeholder">Your words land here…</span>';
  $('#liveFillerCount').textContent = '0';
  $('#ghostFace').textContent = '👻';
  const c = $('#waveCanvas');
...
async function beginSession() {
  if (!SR) {
    micWarn("This browser can't do speech recognition. Chrome or Edge will.");
    showView('drills');
    return;
  }
  try {
    ...
    micWarn('Cadence needs your mic to play. Allow it and try again.');
    showView('drills');
    return;
  }
  ...
  $('#recordBtnLabel').textContent = '⏹ Done';
...
function onSpeechResult(e) {
  ...
  if (el.textContent !== String(n)) {
    el.textContent = n;
    const tile = $('#ghostTile');
    $('#ghostFace').textContent = n >= 6 ? '😈' : n >= 3 ? '👿' : '👻';
...
function drawWave() {
  ...
  function frame() {
    ...
    for (let i = 0; i < bars; i++) {
      ...
      const h = Math.max(8, v * canvas.height * 0.8);
      const x = i * bw + bw * 0.2;
      const y = (canvas.height - h) / 2;
      ctx.fillStyle = CANDY[i % CANDY.length];
      ctx.beginPath();
      ctx.roundRect(x, y, bw * 0.6, h, 6);
      ctx.fill();
    }
...
function gradeFor(score) {
  if (score >= 85) return 'Certified yapper. 🏆';
  if (score >= 70) return 'Pretty smooth! 🎉';
  if (score >= 50) return 'Getting there. 💪';
  return 'Beautiful chaos. 🌪️';
}

function tipFor(r) {
  if (r.words < 10) return 'The mic barely heard you. Scoot closer, speak up, run it back.';
  if (r.fillerRate > 5) return `The ghost ate ${r.fillers} fillers in ${r.words} words. Next round, swap each one for a tiny silent pause. Silence sounds smart.`;
  if (r.pauses >= 3) return `You froze ${r.pauses} times for 2+ seconds. Trick: decide how the sentence ends before you start it.`;
  if (r.wpm < 110) return `${r.wpm} wpm is bedtime-story pace. Push faster, speed forces your brain to pre-edit.`;
  if (r.wpm > 210) return `${r.wpm} wpm! Auctioneer energy. Check the transcript still makes sense.`;
  return 'Fast, clean, zero stalls. Shuffle to a topic you know nothing about and try to keep this up.';
}
...
function showResults() {
  ...
  $('#resultTranscript').innerHTML = transcript
    ...
    : '<i>No speech detected. Check your mic and run it back.</i>';

  // tile color by score
  const tile = $('#scoreTile');
  tile.style.background = r.score >= 85 ? '#00a551' : r.score >= 70 ? '#2b46f0' : r.score >= 50 ? '#6c1ee0' : '#f2055c';

  animateNumber($('#scoreNum'), r.score, 1000);
  if (r.score >= 85) confetti();
...
// tiny DOM confetti for big scores
function confetti() {
  for (let i = 0; i < 36; i++) {
    const p = document.createElement('span');
    p.textContent = ['🎉', '✨', '⚡', '👏'][i % 4];
    p.style.cssText = `position:fixed;left:${10 + Math.random() * 80}vw;top:-5vh;font-size:${16 + Math.random() * 22}px;z-index:300;pointer-events:none;transition:transform ${1.6 + Math.random()}s cubic-bezier(.2,.6,.4,1),opacity .4s ${1.4 + Math.random()}s;`;
    document.body.appendChild(p);
    requestAnimationFrame(() => {
      p.style.transform = `translateY(${105 + Math.random() * 10}vh) rotate(${(Math.random() - 0.5) * 540}deg)`;
      p.style.opacity = '0';
    });
...
function renderStreak() {
  const h = history();
  const el = $('#streakLine');
  if (!h.length) { el.textContent = "First round. Let's get a baseline."; return; }
  ...
  el.textContent = streak > 1
    ...
    : `${h.length} round${h.length > 1 ? 's' : ''} played · last score ${last.score}/100`;
}
...
function renderProgress() {
  ...
  if (!h.length) {
    sum.textContent = 'Nothing yet. Go make a number.';
    return;
  }
  ...
  sum.textContent = `${h.length} round${h.length > 1 ? 's' : ''} · all-time avg ${avg} · recent avg ${recentAvg}`;

  // candy lollipop chart: stems + dots
  const pts = h.slice(-30);
  const W = canvas.width, H = canvas.height, pad = 26;
  ctx.strokeStyle = 'rgba(22,21,19,0.12)';
  ctx.lineWidth = 2;
  ctx.beginPath();
  ...
  ctx.stroke();
  pts.forEach((s, i) => {
    const x = pad + ((i + 0.5) / pts.length) * (W - pad * 2);
    const y = H - pad - (s.score / 100) * (H - pad * 2.2);
  ...
  [...h].reverse().slice(0, 12).forEach(s => {
    ...
    row.innerHTML =
      ...
      `<span>${s.wpm} wpm · ${s.fillerRate} fill</span>` +
